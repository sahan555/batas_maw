...
const ResaleInquiry = () => {
  ...
  const { post } = usePost("resale-inquiry-store");
  const initialValues = {
    name: "",
    email: "",
    phone: "",
  ...
  const handleSubmit = async (values, { resetForm }) => {
    setLoader(true);
    await post({ ...values, resale_id: details?.id }, "Resale Form");
    resetForm();
    setLoader(false);
  };
          ...
          <div className="container mx-auto">
            <div className="heading-wrapper">
              <h1 className="heading mb-4 !text-primary">
                {details?.resale_product_name} <span className="text-black">(Inquiry Form)</span>
              </h1>
            </div>
